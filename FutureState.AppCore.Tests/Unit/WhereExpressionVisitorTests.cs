...
namespace FutureState.AppCore.Tests.Unit
{
    ...
    public class WhereExpressionVisitorTests
    {
        ...
        public void ShouldBuildAWhereClauseWithAnOr()
        {
            ...
            var expectedParameters = new Dictionary<string, object> { { "@FirstName1", "%" + name + "%" }, { "@LastName1", "%" + name + "%" } };
            var actualString = "";
            ...
            {
                var actualExpression = TestExpression<StudentModel>( u => u.FirstName.Contains( item ) || u.LastName.Contains( item ) );
