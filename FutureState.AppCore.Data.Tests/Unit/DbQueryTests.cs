...
namespace FutureState.AppCore.Data.Tests.Unit
{
    ...
    public class DbQueryTests : DbQueryTestBase
    {
        ...
        {
            // setup
            const string expectedQuery = @"SELECT Students.* FROM Students WHERE [FirstName] LIKE @FirstName1 AND [FirstName] <> @FirstName2";
        ...
        public void ShouldBuildQueryWithOrderByAscClause ( IDbProvider dbProvider )
        {
            ...
            // setup
            const string expectedQuery = @"SELECT Students.* FROM Students WHERE [FirstName] LIKE @FirstName1 ORDER BY [Email] ASC";
        ...
        public void ShouldBuildQueryWithOrderByDescClause ( IDbProvider dbProvider )
        {
            // TODO: Joe to implement
            // setup
            const string expectedQuery = @"SELECT Students.* FROM Students WHERE [FirstName] LIKE @FirstName1 ORDER BY [Email] DESC";
        ...
        public void ShouldBuildQueryForUsersAndUtilizeSkipTake(IDbProvider dbProvider)
        {
            ...
            switch (dbProvider.GetType().ToString())
            {
                ...
                case "FutureState.AppCore.Data.SqlServer.DbProvider":
                    expectedQuery = @"SELECT Students.* FROM Students WHERE [Id] = @Id1  OFFSET 20 ROWS FETCH NEXT 10 ROWS ONLY";
                    break;
            }
